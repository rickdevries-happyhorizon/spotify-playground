...
def manage_ui_skin():
    ...
    while True:
        ...
        print(f"  {Colors.BRIGHT_GREEN}1.{Colors.RESET} neon  {Colors.DIM}(animated gradients and glow effects){Colors.RESET}")
        print(f"  {Colors.BRIGHT_GREEN}2.{Colors.RESET} simple  {Colors.DIM}(black and white, flat layout){Colors.RESET}")
        print(f"  {Colors.DIM}0.{Colors.RESET} Back to main menu")
        print(f"\n{Colors.DIM}{'-' * 70}{Colors.RESET}")

        try:
            choice = input(f"{Colors.BRIGHT_CYAN}Enter your choice (0-2): {Colors.RESET}").strip()
